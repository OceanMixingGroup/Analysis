...
%DIF DEL ChipodMethods_copy_Jul18.tex   Fri Jul 15 10:08:12 2016
%DIF ADD ChiPodMethods.tex              Mon Jul 25 10:01:29 2016
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\title{Estimating $\chi$ \DIFdelbegin \DIFdel{and $K_T$ from fast-reponse }\DIFdelend \DIFaddbegin \DIFadd{using fast-response }\DIFaddend thermistors on traditional shipboard CTDs: sources of uncertainty and bias.}
...
%DIF < \email{}
%DIF -------
\email{apickering@coas.oregonstate.edu} %DIF > 
%DIF -------
...
%DIF 84c84
%DIF <     \extraauthor{Jim Moum}
%DIF -------
    \extraauthor{James N Moum} %DIF > 
%DIF -------
    \extraaffil{College of Earth, Ocean, and Atmospheric Sciences, Oregon State University,Corvallis, OR.}

%DIF 87c87
%DIF <     \extraauthor{Jen MacKinnon}
%DIF -------
    \extraauthor{Jennifer MacKinnon} %DIF > 
%DIF -------
...
\DIFdelbegin \DIFdel{Direct measurement of mixing }\DIFdelend \DIFaddbegin \DIFadd{The most accurate measurements of turbulence are made }\DIFaddend with microstructure profilers equipped with shear probes\DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{.  Shear probes are used to measure the spectra of velocity shear at small enough scales that a fit to standard inertial subrange spectral shapes allows an estimate of the turbulent dissipation rate ($\epsilon$) as a fitting parameter. This technique is well-suited for targeting upper-ocean processes, but can be }\DIFaddend expensive, time-intensive, and requires considerable care and expertise. Moreover, tethered profilers can't reach abyssal depths, requiring autonomous instruments to get deeper than $\sim$1000-2000 m.  As a result, existing measurements of diapycnal mixing, especially in the deep ocean, are sparse \citep{waterhouseetal14}. In order to obtain a larger quantity of mixing estimates, considerable work has gone into inferring mixing from measurements of the outer scales of turbulence, which are easier to obtain. One popular method is the use of Thorpe scales, where diapycnal mixing is inferred from inversions in profiles of temperature or density  \citep{thorpe77,dillon82}. The size of resolvable overturn is limited by the profiling speed and instrument noise \citep{galbraithkelley96}. 
\DIFdelbegin \DIFdel{Several }\DIFdelend \DIFaddbegin \DIFadd{While some }\DIFaddend studies indicate relatively good agreement with microstructure and other observations, \DIFdelbegin \DIFdel{but there are some }\DIFdelend \DIFaddbegin \DIFadd{there remain }\DIFaddend questions about the \DIFaddbegin \DIFadd{general }\DIFaddend validity of the method and the assumptions made \citep{materetal15,scotti15}. Parameterizations based on profiles of shear and/or strain have also been developed and applied to estimate diapycnal mixing \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{gregg89a,kunzeetal06,polzinetal13,whalenetal12,whalenetal15}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{gregg89a,kunzeetal06,polzinetal14,whalenetal12,whalenetal15}
}%DIFAUXCMD
}\DIFaddend .  However, they rely on a series of assumptions about the cascade of energy from large to small scales \DIFaddbegin \DIFadd{(they assume turbulence rates are set by an energy cascade through the internal wave field) }\DIFaddend that are often violated; numerous studies (i.e., \cite{watermanetal13}) have shown that there is significant uncertainty associated with these parameterizations, in that there can be a consistent bias in a particular region, yet the sense of the bias (i.e., \DIFdelbegin \DIFdel{overpredict }\DIFdelend \DIFaddbegin \DIFadd{over-predict }\DIFaddend vs.\ \DIFdelbegin \DIFdel{underpredict}\DIFdelend \DIFaddbegin \DIFadd{under-predict}\DIFaddend ) is not known a priori. 

\DIFdelbegin \DIFdel{Quanitifying }\DIFdelend \DIFaddbegin \DIFadd{Quantifying }\DIFaddend turbulence from velocity shear variance (to compute the dissipation rate of turbulent kinetic energy $\epsilon$) is challenging on moorings or profiling platforms because there is usually too much vibration and/or package motion for shear-probes to be useful. \DIFaddbegin \DIFadd{Pitot-static tubes have recently been used for this purpose, plus providing independent measurements of speed \mbox{%DIFAUXCMD
\citep{moum15}
}%DIFAUXCMD
. }\DIFaddend Other methods (i.e., optics or acoustics) may hold some promise, but lack of scatterers often precludes this type of measurement, especially in the abyss.  In addition, shear probes only provide $\epsilon$, not the \DIFdelbegin \DIFdel{mixing }\DIFdelend \DIFaddbegin \DIFadd{diapycnal diffusivity }\DIFaddend of scalars, $K$, which is often inferred from $\epsilon$ by assuming a mixing efficiency $\Gamma$ \citep{osborn80} as $K=\Gamma \epsilon / N^{2}$, \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $N^2$ is the buoyancy frequency.  A more direct measure of turbulent mixing is obtained from the dissipation rate of temperature variance $\chi$ \citep{osborncox72}.  This has the advantage that (i) the temperature and temperature gradient can be computed and are relatively straightforward to measure, and (ii) the estimation of mixing from $\chi$ does not require assumptions about $\Gamma$. However, the spectrum of temperature gradient extends to very small scales, so that its spectrum is seldom fully resolved (and unlike shear variance, the wavenumber extent of the temperature gradient spectrum does not scale with its amplitude, but instead depends on $\epsilon$). Assumptions about the spectral shape (Kraichnan vs.\ Bachelor, and the value of the ``constant'' q) and its wavenumber extent (governed by the Batchelor wavenumber $k_b=[\epsilon/(\nu D_{T}^{2})]^{1/4}$  \citep{Batchelor59}) are thus necessary to determine $\chi$ unless measurements capture the full viscous-diffusive subrange of turbulence (i.e., down to scales $\Delta x \sim 1/k_b \sim 1$mm), a criterion seldom achieved.  To resolve this, we follow \cite{alfordpinkel00b} and \cite{moumnash09} and make the assumption that $K_T=K_{\rho}$ to determine the dissipation rate as $\epsilon_{\chi}=(N^2\chi)/(2\Gamma <dT/dz>^2)$, permitting \DIFaddbegin \DIFadd{estimation of }\DIFaddend $k_b$\DIFdelbegin \DIFdel{to be estimated}\DIFdelend . 


The goal of this paper is to outline and validate the methods used to compute $\chi$ and $K_T$ with $\chi$\DIFdelbegin \DIFdel{-pods }\DIFdelend \DIFaddbegin \DIFadd{pods }\DIFaddend mounted on CTDs \DIFaddbegin \DIFadd{(Figure \ref{f1})}\DIFaddend .  We do this by applying our processing methodology to profiles of temperature gradient measured by thermistors on the `Chameleon' microstructure profiler, which provides a direct test of our methodology.  Because Chameleon is a loosely tethered profiler equipped with shear probes \DIFdelbegin \DIFdel{(Moum et al 1995)}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{moumetal95}
}%DIFAUXCMD
}\DIFaddend , it directly measures $\epsilon$ and allows us to test our assumptions.  Specifically, it allows us to determine biases associated with computing $\chi$ from partially-resolved temperature \DIFaddbegin \DIFadd{gradient spectra }\DIFaddend alone, as compared to \DIFdelbegin \DIFdel{that when it is computed by including knowledge of the dissipation rate}\DIFdelend \DIFaddbegin \DIFadd{computation that includes $\epsilon$}\DIFaddend , which constrains the wavenumber extent of the scalar spectra.   After establishing that the method works, we then compare CTD-$\chi$pod profiles to nearby microstructure profiles.
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{where $u$ represents the flow speed past the sensor. }\DIFaddend The wavenumber extent of the spectrum depends on the Batchelor wavenumber $k_b$, which is related to $\epsilon$:
\begin{equation}
k_b=[\epsilon/(\nu D_{T}^{2})]^{1/4}
\label{eq:3}
\end{equation}

We assume that $K_{\rho}=K_T$ and $K_{\rho}=\Gamma \epsilon /N^2$ \DIFaddbegin \DIFadd{where the mixing efficiency $\Gamma$ is assumed to be $0.2$ \mbox{%DIFAUXCMD
\citep{moumnash09}
}%DIFAUXCMD
}\DIFaddend . Then dissipation rate is computed as
\begin{equation}
\label{eq:eps}
\epsilon_{\chi}=\frac{N^2\chi_T}{2\Gamma <dT/dz>^2}
\end{equation}

Typical thermistors do not resolve the spectrum out to $k_b$, so the measured spectrum is fit to the Kraichnan form \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{(with $q=7$) of the }\DIFaddend theoretical scalar spectrum over the range of resolved wavenumbers ($k_{min}<k<k_{max}$). The variance between the measured $[\Phi_{T_x}(k)]_{obs}$ and theoretical $[\Phi_{T_x}(k)]_{theory}$ spectra at these wavenumbers is assumed to be equal:
...
\begin{enumerate}
\item First we estimate $\chi_T$ based on an initial guess of $\epsilon=10^{-7}$ $\mathrm{Wkg^{-1}}$ and compute $k_b$ via eq. \ref{eq:3}. We set $k_{max} = k_b/2$ or to a wavenumber equivalent to $f_{max}=7$ Hz [i.e., $k_{max}= 2\pi(f_{max})/u$], whichever is smaller. In general \DIFdelbegin \DIFdel{$fmax$ }\DIFdelend \DIFaddbegin \DIFadd{$f_{max}$ }\DIFaddend should be the highest value which is safely below the sensor's roll-off. We chose $f_{max}=7$Hz in this case based on inspection of the temperature gradient spectra and historical measurements of these sensors (see appendix for more details).
...
\end{enumerate}
Note that this procedure is equivalent to the explicit formulation of \citep{alfordpinkel00b}, except we use the Kraichnan theoretical form instead of the Batchelor spectrum for $[\Phi_{T_x}(k)]_{theory}$. \DIFaddbegin \DIFadd{At wavenumbers below the spectral peak, there is little distinction between the Kraichnan and Batchelor spectra, so this factor does not affect the computational bias.
}\DIFaddend 

\subsection{CTD-$\chi$pod Data Processing}

\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We next review the }\DIFaddend basic outline for processing each CTD-$\chi$\DIFdelbegin \DIFdel{-pod profileis as follows:
}\DIFdelend \DIFaddbegin \DIFadd{pod profile. The moored $\chi$pod instrument \mbox{%DIFAUXCMD
\citep{moumnash09}
}%DIFAUXCMD
has a pressure sensor, compass, and pitot-static tube. In contrast, the CTD-$\chi$pod requires pressure measured by the CTD and has no independent speed measurement other that $dp/dt$ from the CTD.
}\DIFaddend \begin{enumerate}
\item The correct time-offset for the $\chi$\DIFdelbegin \DIFdel{-pod }\DIFdelend \DIFaddbegin \DIFadd{pod }\DIFaddend clock is determined by aligning highpass-filtered $dp/dt$ from the 24Hz CTD data to  integrated vertical accelerations measured by the $\chi$\DIFdelbegin \DIFdel{-pod}\DIFdelend \DIFaddbegin \DIFadd{pod}\DIFaddend . $\chi$\DIFdelbegin \DIFdel{-pod }\DIFdelend \DIFaddbegin \DIFadd{pod }\DIFaddend clock drift is small, typically on the order of 1 sec/week, but it is imperative to get records aligned within $< 0.5$ s so that the correct \DIFdelbegin \DIFdel{$u = w = d p/dt$ }\DIFdelend \DIFaddbegin \DIFadd{value of $u$ }\DIFaddend is used. \DIFaddbegin \DIFadd{In the case of the CTD-$\chi$pod we assume $u$ is solely due to the vertical motion of the CTD cage, i.e. $u=dp/dt$.
}\DIFaddend \item Low-order polynomial calibration coefficients are determined to convert thermistor voltages from $\chi$pod to ITS90 temperature (as measured by
...
\item Depth loops are identified and flagged in the 24Hz CTD data \DIFdelbegin \DIFdel{Figure \ref{depthloops2}}\DIFdelend \DIFaddbegin \DIFadd{(Figure \ref{depthloops2})}\DIFaddend . $\chi$\DIFdelbegin \DIFdel{-pod }\DIFdelend \DIFaddbegin \DIFadd{pod }\DIFaddend data during these times are discarded since the signals are likely contaminated by the wake of the CTD. We use a \DIFdelbegin \DIFdel{veritcal }\DIFdelend \DIFaddbegin \DIFadd{vertical }\DIFaddend velocity threshold of $0.3$m/s and throw out data within 2m of the identified loops. Even for profiles that are significantly affected by ship heaving, good segments of data are obtained over a majority of the depths after removing contaminated data, allowing us to compute values in nearly every 10m bin. 
...
\item Half-overlapping 1 sec windows of data are used to estimate $\chi$ \DIFdelbegin \DIFdel{and $K_T$ }\DIFdelend following the methods described in \cite{moumnash09}, as outlined in the previous section.
\end{enumerate}
...
Examples of the observed \DIFaddbegin \DIFadd{(from FP07 thermistor on Chameleon) }\DIFaddend and fit spectra are shown in Figure \ref{specexamp}, for \DIFaddbegin \DIFadd{two windows where a }\DIFaddend low and high dissipation rate \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{was observed. The Chameleon spectra (magenta) shown is the theoretical Kraichnan spectra for the values of $\chi$ and $\epsilon$ estimated using the shear-probe data. The $\chi$pod fit is the estimate from applying the $\chi$pod method to just the thermistor data for the same window. }\DIFaddend Note that at lower $\epsilon$, a larger fraction of $k_b$ is observed and the peak of the spectrum is \DIFdelbegin \DIFdel{nearly }\DIFdelend \DIFaddbegin \DIFadd{almost }\DIFaddend resolved. At higher $\epsilon$, less of the spectrum is resolved and the spectral peak is well above the maximum resolved wavenumber. Even so, the iterative $\chi$pod method gives an accurate estimate of $\chi$. The $\chi$pod and Chameleon fits are performed over the same wavenumber range and \DIFaddbegin \DIFadd{thus }\DIFaddend match there. However, the higher-wavenumber \DIFdelbegin \DIFdel{protions }\DIFdelend \DIFaddbegin \DIFadd{portions }\DIFaddend of the fit spectra differ since the \DIFdelbegin \DIFdel{Chameelon }\DIFdelend \DIFaddbegin \DIFadd{Chameleon }\DIFaddend fits use the observed $k_b$ to determine the wavenumber extent.
...
\DIFdelend %DIF > \subsection{flowspeed past the sensor}
...
We begin by utilizing the highly-resolved \DIFdelbegin \DIFdel{turbulence profiler data }\DIFdelend \DIFaddbegin \DIFadd{data from the freely-falling turbulence profiler, Chameleon }\DIFaddend (for which both $\epsilon$ and $\chi$ are measured) to test the assumptions in our method of estimating $\chi$. We first apply the $\chi$pod method to each Chameleon profile, using only the FP07 thermistor data. These results, which we \DIFdelbegin \DIFdel{will }\DIFdelend refer to as \DIFdelbegin \DIFdel{$\chi_{\chi}$}\DIFdelend \DIFaddbegin \DIFadd{$\chi_{\chi}^{cham}$ and assume no independent knowledge of $\epsilon$}\DIFaddend , are then compared \DIFdelbegin \DIFdel{with $\chi_{\epsilon}$}\DIFdelend \DIFaddbegin \DIFadd{to $\chi_{\epsilon}^{cham}$}\DIFaddend , computed by integrating the theoretical temperature gradient spectrum \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $k_b$ \DIFaddbegin \DIFadd{is }\DIFaddend computed directly from shear-probe derived $\epsilon$. Qualitatively, \DIFdelbegin \DIFdel{$\chi_{\chi}$ and $\chi_{\epsilon}$ }\DIFdelend \DIFaddbegin \DIFadd{$\chi_{\chi}^{cham}$ and $\chi_{\epsilon}^{cham}$ }\DIFaddend show very similar depth and time patterns (Figure \ref{eq14_eps_pcolor}) and appear to agree in magnitude\DIFdelbegin \DIFdel{, suggesting the method works well}\DIFdelend . A more quantitative comparison \DIFdelbegin \DIFdel{is made with a 2-D histogram }\DIFdelend (Figure \ref{eq14_chi_2dhist}), \DIFdelbegin \DIFdel{showing }\DIFdelend \DIFaddbegin \DIFadd{shows }\DIFaddend the two are well-correlated over \DIFdelbegin \DIFdel{a wide range of magnitudes}\DIFdelend \DIFaddbegin \DIFadd{five orders of magnitude}\DIFaddend . The distribution of $log_{10}$ of the $\chi$ ratios is approximately normal, with a mean of $\mu=-0.1$ and standard deviation of $\sigma=0.51$. \DIFaddbegin \DIFadd{This indicates a low bias of $\chi_{\chi}^{cham}$ relative to $\chi_{\epsilon}^{cham}$ of 20}\% \DIFadd{and random variation of a factor of 3. 
}\DIFaddend 
...
We have shown that $\chi$ can be accurately estimated from $\chi$pods attached to CTD rosettes. The method also estimates \DIFaddbegin \DIFadd{$K_T$ and }\DIFaddend $\epsilon$, but we have \DIFdelbegin \DIFdel{not discussed it here since it involves }\DIFdelend \DIFaddbegin \DIFadd{left discussion of these for a future paper since they involve }\DIFaddend more assumptions and uncertainties. One major assumption is the mixing efficiency $\Gamma$. A value of $0.2$ is commonly assumed, but evidence suggests this may vary significantly. \cite{moumnash09} found a bias \DIFaddbegin \DIFadd{in $\chi$ }\DIFaddend of up to 1.6 for $\Gamma$ values ranging from $0.1$ to $0.35$.  Another major assumption in the $\chi$pod method is that $K_T=K_{\rho}$\DIFdelbegin \DIFdel{. Preliminary investigation on this data suggests that this assumption does not hold here and may be responsible for a negative bias in $\epsilon$, but more work is needed to verify this. 
However we have shown that even if $\epsilon$ estimates are biased, $\chi$ and $K_T$ are still robust.
}\DIFdelend \DIFaddbegin \DIFadd{, which should be true where density is dominated by temperature. 
}\DIFaddend 

The goal of CTD-$\chi$pods is to expand the number and spatial coverage of ocean mixing observations. \DIFaddbegin \DIFadd{The census of \mbox{%DIFAUXCMD
\cite{waterhouseetal14}
}%DIFAUXCMD
found 10 locations where full-depth microstructure profiles were taken (see their figure 1c), all of which had between 11 and 100 profiles. }\DIFaddend We have already deployed \DIFdelbegin \DIFdel{instruments }\DIFdelend \DIFaddbegin \DIFadd{CTD $\chi$pods }\DIFaddend during several process experiments and on several GO-SHIP repeat-hydrography cruises\DIFaddbegin \DIFadd{, obtaining more than 1700 full-depth profiles of $\chi$ over a wide range of locations}\DIFaddend . We plan to continue regular deployment on GO-SHIP and similar cruises, adding $\chi$ \DIFdelbegin \DIFdel{and $K_T$ }\DIFdelend to the suite of variables regularly measured. The expanding database of mixing measurements from CTD-$\chi$pods will also enable testing of other commonly-used or new mixing parameterizations. This has the potential to be transformative for the field, allowing the community to develop and test global turbulence parameterizations, use estimates of turbulence along with the CLIVAR repeat hydrography data for inverse models and water mass modification calculations, identify hotspots of turbulence to target with future process experiments, and compare with in-situ chemical and biological measurements made routinely on repeat hydrography cruises.
...
\item We conclude that estimates of $\chi$ \DIFdelbegin \DIFdel{and $K_T$ }\DIFdelend made from the CTD-$\chi$pod platform are robust and reliable.
\end{itemize}
...
%
\acknowledgments
The EQ14 experiment was funded by \DIFdelbegin \DIFdel{???. }\DIFdelend \DIFaddbegin \DIFadd{budget (NSF?) 1256620. }\DIFaddend We thank the Captain and crew of R/V Oceanus. \DIFdelbegin \DIFdel{Also thank people who helped collect the data, those who }\DIFdelend \DIFaddbegin \DIFadd{Pavan Vutukur, Craig Van Appledorn, Mike Neeley-Brown }\DIFaddend made the sensors\DIFdelbegin \DIFdel{, etc.
.
}\DIFdelend \DIFaddbegin \DIFadd{. Aurelie Moulin, Ryan Holmes, Sally Warner and Anna Savage helped to collect the data.
}\DIFaddend 
...
To \DIFdelbegin \DIFdel{quanitify }\DIFdelend \DIFaddbegin \DIFadd{quantify }\DIFaddend the potential error in the \DIFaddbegin \DIFadd{CTD-}\DIFaddend $\chi$pod calculations from ignoring horizontal velocities and assuming the flow speed is equal to the vertical speed of the CTD rosette, we repeated the calculations with constant offsets added to the flowspeed. \DIFdelbegin \DIFdel{Since }\DIFdelend \DIFaddbegin \DIFadd{Note that since }\DIFaddend the total magnitude of velocity is used, $dp/dt$ is a minimum estimate of the true speed. Adding $0.1$($1$)m/s results in a mean percent error of \DIFdelbegin \DIFdel{14(58}\DIFdelend \DIFaddbegin \DIFadd{-14(-58}\DIFaddend ) percent (Figure \ref{FspdSensHist}), small compared the large natural variability in turbulence and uncertainty in our measurements. Note that increasing the velocity tends to result in smaller values of $\chi$, since it shifts the spectrum to lower wavenumbers. 

We also looked for any systematic biases associated with flowpseed. We found \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend $\chi$ was biased high for very small speeds ($u<25$cm/s). This could be \DIFdelbegin \DIFdel{assoicated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend with contamination by CTD wake or entrained water when the CTD slows.  These values were discarded for our analysis.

%DIF > When the CTD package is equipped with LADCP, the true flowspeed can be measured. In some cases where the CTD was not equipped with LADCP, it would seem that the ship's ADCP could be used to estimate the horizontal component of velocity. However, because the CTD does not travel perfectly vertically in strong currents, this is difficult to do in practice. In a strong horizontal flow, the CTD will drift with the current while descending, lowering the flow relative to the sensors. On the upcasts, the CTD will be pulled against the current, increasing the flow relative to the sensors.
%\appendix[B]
...
\DIFdelbegin %DIFDELCMD < \appendix[D]
%DIFDELCMD < %%%
...
%DIFDELCMD <   \caption{%%%
\DIFdel{Portion of a CTD cast showing depth vs time (top) for a portion of an upcast, with data flagged as loops in red. Lower panel shows corresponding $\chi$pod timeseries of $dT/dt$ for that time with discarded loop data in red. Note that variance tends to be larger during depth loops due to entrained water and CTD-induced turbulence.}%DIFDELCMD < }
%DIFDELCMD <   \label{depthloops2}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % PlotExampleTS.m
\begin{figure}[t]
  \noindent\includegraphics[width=38pc,angle=0]{SN1001_Cast02_Fig5_T_P_dTdz_fspd.png}\\
  \caption{Example timeseries from one CTD cast during EQ14. a) CTD pressure. b) Fallspeed of CTD ($dp/dt$) .c) Vertical and horizontal accelerations measured by $\chi$\DIFdelbeginFL \DIFdelFL{-pod}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pod}\DIFaddendFL . d) Temperature from CTD and (calibrated) $\chi$\DIFdelbeginFL \DIFdelFL{-pod }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pod }\DIFaddendFL sensors. T2 is offset slightly for visualization. e) Temperature derivative $dT/dt$ measured by the upward-looking $\chi$\DIFdelbeginFL \DIFdelFL{-pod }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pod }\DIFaddendFL sensor T1. f) Temperature derivative $dT/dt$ measured by the downward-looking $\chi$\DIFdelbeginFL \DIFdelFL{-pod }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pod }\DIFaddendFL sensor T2.}
  \label{f2}
\end{figure}

\begin{figure}[t]
  \noindent\DIFaddbeginFL \includegraphics[width=38pc,angle=0]{DepthLoopExample2TS.png}\\
  \caption{\DIFaddFL{Portion of a CTD cast showing depth vs time (top) during an upcast, with data flagged as loops in red. Lower panel shows corresponding $\chi$pod timeseries of $dT/dt$ for that time with discarded loop data in red. Note that variance tends to be larger during depth loops due to entrained water and CTD-induced turbulence.}}
  \label{depthloops2}
\end{figure}


\begin{figure}[t]
  \noindent\DIFaddendFL \includegraphics[width=38pc,angle=0]{ExSpec_HiLowEps.png}\\
  \caption{\DIFdelbeginFL \DIFdelFL{Example }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Two example }\DIFaddendFL temperature gradient spectra from \DIFaddbeginFL \DIFaddFL{an }\DIFaddendFL EQ14 \DIFaddbeginFL \DIFaddFL{Chameleon profile}\DIFaddendFL , for high (top) and low (bottom) \DIFaddbeginFL \DIFaddFL{values of }\DIFaddendFL $\epsilon$. Solid black \DIFdelbeginFL \DIFdelFL{line }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{lines }\DIFaddendFL with circles show the observed \DIFaddbeginFL \DIFaddFL{temperature-gradient }\DIFaddendFL spectra \DIFaddbeginFL \DIFaddFL{from the thermistor on Chameleon}\DIFaddendFL . Dashed \DIFaddbeginFL \DIFaddFL{black }\DIFaddendFL line shows the fitted theoretical Kraichnan spectra \DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{estimated by applying }\DIFaddendFL the $\chi$pod \DIFdelbeginFL \DIFdelFL{estimates}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{method to the thermistor data only}\DIFaddendFL . Magenta line is Kraichnan spectra for Chameleon $\chi$ and $\epsilon$ measured at the same depth\DIFaddbeginFL \DIFaddFL{, using the shear probe data as well}\DIFaddendFL . Vertical dashed blue(red) lines indicate the minimum (maximum) wavenumber used in the $\chi$pod calculation. The Batchelor wavenumber $k_b$ is also indicated by the vertical \DIFdelbeginFL \DIFdelFL{cyan }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{magenta }\DIFaddendFL line.}
  \label{specexamp}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Depth-time plots of $log_{10}\chi$ from both methods for EQ14 data. Top: $\chi$pod method \DIFaddbeginFL \DIFaddFL{using only FP07 data from Chameleon}\DIFaddendFL . Black diamonds indicate casts used for comparison with CTD-$\chi$pod profiles. Bottom: Chameleon \DIFaddbeginFL \DIFaddFL{measurements using FP07 and shear probe data}\DIFaddendFL .}
...
\begin{figure}[t]
  ...
  \caption{Left: 2D histogram of $log_{10}(\chi)$ from Chameleon (x-axis) and $\chi$pod method (y-axes). Values from each profile were averaged in the same 5m depth bins. Right: Normalized histogram of \DIFdelbeginFL \DIFdelFL{$\log_{10}[\chi_{\chi}/\chi_{\epsilon}]$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\log_{10}[\chi_{\chi}^{cham}/\chi_{\epsilon}^{cham}]$}\DIFaddendFL . Vertical dashed line indicates the mean of the the distribution.}
...
\begin{figure}[t]
  ...
  \caption{Histogram of $log_{10}$ of the ratio of $\chi$ for nearby casts. The first set is for the before (\DIFdelbeginFL \DIFdelFL{cham1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\chi_{\epsilon1}^{cham}$}\DIFaddendFL ) and after (\DIFdelbeginFL \DIFdelFL{cham2) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\chi_{\epsilon2}^{cham}$ }\DIFaddendFL Chameleon profiles. \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The }\DIFaddendFL 2nd is CTD-$\chi$pod profiles \DIFaddbeginFL \DIFaddFL{($\chi_{\chi}^{ctd}$) }\DIFaddendFL versus the before(\DIFdelbeginFL \DIFdelFL{cham1}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\chi_{\epsilon1}^{cham}$}\DIFaddendFL ) profiles. The last is CTD-$\chi$pod profiles \DIFaddbeginFL \DIFaddFL{($\chi_{\chi}^{ctd}$) }\DIFaddendFL versus the after(\DIFdelbeginFL \DIFdelFL{cham2}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\chi_{\epsilon2}^{cham}$}\DIFaddendFL ) profiles. Dashed lines show the medians of each set.  Note that bias is small/zero, and the variability (spread) between CTD/cham is similar to the natural variability between cham profiles.}
...
%DIF <  \caption{Example chipod data from P16N. Top: dTdz. Bottom: $\chi$.}
%DIF <  \label{p16ex}
%DIF < \end{figure}
\DIFdelbegin %DIFDELCMD < 
...
\DIFdelend \begin{figure}[t]
  ...
  \caption{Histogram of \% error for $\chi$ computed with constant added to fallspeed, in order to examine sensitivity to fallspeed. \DIFaddbeginFL \DIFaddFL{Vertical lines indicate the mean of each distribution.}\DIFaddendFL }
  \label{FspdSensHist}
\end{figure}

\begin{figure}[t]
  \noindent\includegraphics[width=38pc,angle=0]{MLEfitsEQ14_Whist.png}\\
  \caption{Left: 2D histograms of $\chi$ computed using the \DIFdelbeginFL \DIFdelFL{interative }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{iterative }\DIFaddendFL $\chi$\DIFdelbeginFL \DIFdelFL{-pod }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{pod }\DIFaddendFL method (top) and the MLE fit (bottom) versus $\chi$ computed from Chameleon. Note that the MLE method underestimates $\chi$ at larger magnitudes. Right: Histograms of the log of ratios for different ranges of $\epsilon$.}
  \label{mlefits}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{Measured FP07 thermistor transfer functions from historical database. Vertical dashed lines show the frequency range used in $\chi$pod method. Dashed blue line is a generic transfer function found to best match measured functions\DIFdelbeginFL \DIFdelFL{, and is used when response functions are no longer measured}\DIFdelendFL .}
  \label{xfr}
\end{figure}
