%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% Compare_chi_diff_zsmooth_EQ08.m
%
% This script looks at effect of 'zsmooth' parameter in chi-pod calculations.
...
%h4=histogram(log10(C4.chi(:)),h1.BinEdges,'DisplayStyle',Ds);
...
legend([ h1  h3 ],'zsm=1m','zsm=10m','location','best')
...
if saveplot==1
    figname=['hist_chi_eps_diff_zsmooth']
...
plot(xbins,xbins+1,'r--')
xlim(xl);ylim(xl)
title(['zsmooth=' num2str(C1.Params.z_smooth) 'hz'])
xlabel('cham : log_{10}\chi ','fontsize',16)
 ylabel('\chi pod method : log_{10}\chi ','fontsize',16)
caxis(cl)
...
plot(xbins,xbins+1,'r--')
xlim(xl);ylim(xl)
title(['zsmooth=' num2str(C3.Params.z_smooth) 'hz'])
xlabel('cham : log_{10}\chi ','fontsize',16)
 ylabel('\chi pod method : log_{10}\chi ','fontsize',16)
caxis(cl)

if saveplot==1
    figname=['2Dhist_chi_diff_zsmooth']
    print(fullfile(figdir,figname),'-dpng')
end
...
if saveplot==1
    figname=['2Dhist_eps_diff_zsmooth']
    print(fullfile(figdir,figname),'-dpng')
end
