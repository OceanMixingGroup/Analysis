%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% compute_thorpe_jn_error_test_AP.m
%
% Starting with compute_thorpe_jn_error_test.m by Jonathan Nash.
%
% Compute overturns for Jonathan's IWISE T-chains, for full data and data
% resampled to approximate MP sawtooth sampling. Goal is to determine
% effect of sampling on overturns and dissipation estimates.
%
...
load('/Users/Andy/Dropbox/ModelObsComparisons/jonathan_T_Chains/mooring3.mat')
...
%xx.T2=interp_missing_data(extrapolate_data(xx.T,'both'),10); % AP 11/8 -
%don't have interp_missing_data
xx.T2=xx.T;
...
%%
%xx.eps_from_interped=xx.eps_from_interped';
...
for tcase=1%[-1 -.66 -.33 0 .33 .66 1]
    ...
    for c=[3]
        ...
        x_oset=round(tcase*45*(linspace(-.5,.5,length(xx.z)))); % AP - this is a depth profile of the offset due to angled sampling
        badi=find(isnan(xx.T));
        %xx.T2=interp_missing_data(extrapolate_data(xx.T,'both'),10); % AP 11/8 -
        %missing this file
        xx.T2=xx.T2;
        for a=1:length(xx.z)
            ...
            maxo=max(abs(x_oset));
            the_inds=(1+maxo):(length(xx.time)-maxo); % AP buffer at begining and end
