%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% Look at just one profile to sort out resampling issues
%
% Based on Misc8Nov.m
%
...
idt=isin(xx.yday,[t_samp(1) t_samp(end)])

figure(1);clf
ezpc(xx.yday(idt),xx.z,xx.T(:,idt))
hold on
plot(t_samp,z_samp,'wo-')
caxis([0 8])
colorbar



[T,Z]=meshgrid(t_real,z_real);
plot(T,Z,'k.')
%
...
% try using interp2 instead for more accurate profile

datai= interp2(t_real(idt),z_real,data_real(:,idt),t_samp,z_samp);

%%

tmid=nanmean(t_samp);
[val,I]=nanmin(abs(tmid-t_real));
% plot rela and resampled profile
figure(2);clf
plot(data_real(:,I),z_real,'k')
hold on
plot(data_resamp,z_samp,'r')
%axis ij
%
%plot(smooth(data_resamp,5),z_samp,'m')
plot(datai,z_samp,'b')
axis ij
...
minOT=10
S_resamp=34.604-.045*(data_resamp-2.5);
[Epsout,Lmin,Lot,runlmax,Lttot]=compute_overturns_discrete(z_samp(:),data_resamp(:),S_resamp(:),35.8,0,minOT,1e-5,0);

S_real=34.604-.045*(data_real(:,I)-2.5);
[Eps_real,Lmin,Lot_real,runlmax,Lttot]=compute_overturns_discrete(z_real(:),data_real(:,I),S_real(:),35.8,0,minOT,1e-5,0);

figure(3);clf

subplot(131)
%semilogx(Epsout,z_samp)
plot(data_real(:,I),z_real,'k')
hold on
plot(data_resamp,z_samp,'r')
axis ij
...
subplot(132)
%semilogx(Epsout,z_samp)
plot(Lot_real,z_real,'k')
hold on
plot(Lot,z_samp,'r')
axis ij

subplot(133)
semilogx(Eps_real,z_real,'ko')
hold on
semilogx(Epsout,z_samp,'ro')
axis ij
grid on
xlim([1e-8 1e-4])
legend('real','resamp')
