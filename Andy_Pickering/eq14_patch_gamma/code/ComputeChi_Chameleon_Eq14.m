...
for icast=1:length(cnums_to_do)
    ...
    close all ; clear cal head tpspec kspec kkspec fspec kks ks
    
    try
        
        % Load the data for this cast
         clear cal ctd head
         load(fullfile(path_chipod_bin,'cal',['zsmooth_' num2str(Params.z_smooth)],[project_short '_' num2str(cnum) '_cal.mat']))
...
%                  pause
        ...
        % save results
        save(fullfile(datdirsave,[project_short '_' sprintf('%04d',cnum) '_avg.mat']),'avg','-v7')
