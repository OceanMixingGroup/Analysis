...
Looking at median values and scatterplots might be a little misleading. Maybe what we care about most is the mean profile of $\epsilon$ (or $K_{\rho}$ and $\chi$ (or $K_T$), which are probably dominated by just a small number of large values. Figure \ref{eps_prof_comp} shows averaged profiles of $\epsilon$ for several 500-profile chunks of the EQ14 data. The $\chi$pod estimates using 1m bins (`bin') tend to overestimate $\epsilon$ (most of the values are actually less than the chameleon values, but apparently the averages are dominated by a few points that are way too high). The $\chi$pod estimates using patches tend to agree well witht he Chameleon profiles.
