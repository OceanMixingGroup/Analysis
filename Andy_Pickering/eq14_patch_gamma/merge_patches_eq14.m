function []=merge_patches_eq14(patch_size_min,usetemp,...
    merge_patches,min_sep)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% merge_patches_eq14.m
%
    ...
    try
        ...
        while ip<Np
            ...
            if ( new_patches(ip,2) - new_patches(ip-1,3)  ) < min_sep % separated by < min_sep
                ...
                new_patches(ip-1,:)=[ new_patches(ip,1) new_patches(ip-1,2) new_patches(ip,3) nan nan nan ];
        ...
        % save profile
        save(fullfile(save_dir_patch,ot_dir,'raw_merge',[project_short '_raw_patches_minOT_' num2str(100*patch_size_min) '_usetemp_' num2str(usetemp) '_cnum_' num2str(cnum) '_merged_minsep_' num2str(min_sep*100) '.mat']), 'patch_data' )
    end % try
end % cnum
